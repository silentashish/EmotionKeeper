...
export default class PieGraph extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    // incase no data loaded in did mount, we do here
  ...
  filterAvailableData(props) {
    // filter color for only available emotions
    const filteredColor = [];
    const filteredData = [];
    this.countData(props).forEach((data) => {
      if (data.y > 0) {
        // if count > 0 then we take
  ...
  renderPie() {
    return (
      <View style={{alignItems: 'center'}}>
        {console.log('Filterd data is', this.state.filteredData)}
        <VictoryPie
          style={{
            labels: {
              color: 'white',
        ...
        <VictoryChart
          height={180}
          width={this.state.filteredData.length * 70}
          ...
          <VictoryBar
            ...
            style={{
              data: {
                fill: (d) => {
                  console.log('value of color ', d);
                  return d.data[d.index].color;
                },
                width: 12,
              },
              labels: {fill: '#ffffff'},
  ...
  render() {
    ...
    return <View style={styles.graphContainer}>{this.renderPie()}</View>;
  }
}
